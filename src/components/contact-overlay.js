...
import s from './contact-overlay.sass'
...
export default class extends Component {
  ...
  render ({ showOverlay, toggleOverlay, content }, { sending, sent, errorSending }) {
    return (
      <div class={cx(s.overlay, !showOverlay && s.hidden)} onClick={this.onClickOverlay} >
        <form class={s.panel} onSubmit={this.onSendMail} ref={el => { this.panelRef = el }}>
          <button type='button' class={s.close} onClick={() => toggleOverlay()}>
          ...
          <h3>{content.contactFormHeader}</h3>
          ...
          {(!sending && !sent) && (
            <div class={s.phoneNumberWrapper}>
              <a class={s.phoneNumber} href={`tel:${content.phoneNumber}`}>
                <svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
              ...
              <p class={s.phoneHours}>{content.phoneHours}</p>
            </div>
          )}
